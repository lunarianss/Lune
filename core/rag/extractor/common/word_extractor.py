...
class WordExtractor(extractor_base.BaseExtractor):
    """Load docx files.
        ...
        for rel in doc.part.rels.values():
            if "image" in rel.target_ref:
                ...
                upload_file = UploadFile(
                    ...
                    storage_type=lune_config.STORAGE_TYPE,
                    key=file_key,
                    name=file_key,
                    size=0,
                ...
                image_map[rel.target_part] = (
                    f"![image]({lune_config.CONSOLE_API_URL}/files/{upload_file.id}/file-preview)"
                )
                ...
                if hasattr(run.element, "tag") and isinstance(run.element.tag, str) and run.element.tag.endswith("r"):
                    ...
                    for drawing in drawing_elements:
                        ...
                        for blip in blip_elements:
                            ...
                            if embed_id:
                                image_part = doc.part.related_parts.get(
                                    embed_id)
                                if image_part in image_map:
                                    paragraph_content.append(
                                        image_map[image_part])
                if run.text.strip():
                    paragraph_content.append(run.text.strip())
            return "".join(paragraph_content) if paragraph_content else ""
